...
data_ip="https://raw.githubusercontent.com/ryuvogs/regip/main/ip"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/xxxx"
    echo -e "      ${GREEN}WhatsApp${NC} wa.me/xxxxxx"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    sleep 5
    reboot
  fi
}
echo -e "${GREENBG}⌛ Memeriksa lisensi...${NC}"
checking_sc
...
# Repository github Tempat File Script
repo="https://raw.githubusercontent.com/ryuvogs/v2/main/"
...
curl -sS ${repo}Fls/password | openssl aes-256-cbc -d -a -pass pass:scvps07gg -pbkdf2 > /etc/pam.d/common-password
...
# install badvpn
cd
wget -O /usr/sbin/badvpn "${repo}Fls/badvpn" >/dev/null 2>&1
chmod +x /usr/sbin/badvpn > /dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn1.service "${repo}Fls/badvpn1.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn2.service "${repo}Fls/badvpn2.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn3.service "${repo}Fls/badvpn3.service" >/dev/null 2>&1
...
wget -O /etc/default/dropbear "${repo}Cfg/dropbear"
...
wget -O /etc/squid/squid.conf "${repo}Cfg/squid3.conf"
...
<font color="#FFEF00">╔═══════════════╗</font><br><font color="#FFE4B5"> 🅡🅨🅤🅥🅞🅖🅢 🅥🅟🅝 </font><br><font color="#FFEF00">╚═══════════════╝
</font><br>
...
<font>
wa.me/6285754707411</b></font>
END

#install bbr dan optimasi kernel
wget -q ${repo}Fls/bbr.sh && chmod +x bbr.sh && ./bbr.sh

wget -q ${repo}Fls/ipserver && chmod +x ipserver && ./ipserver
# blokir torrent
...
# Ambil Xray Core Versi Terbaru Jer
...
clear
cd
wget -q -O /usr/bin/limit-ip "https://raw.githubusercontent.com/ryuvogs/v2/main/lim/limit-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' limit-ip
clear
wget -q -O /usr/bin/limit-ip-ssh "https://raw.githubusercontent.com/ryuvogs/v2/main/lim/limit-ip-ssh"
chmod +x /usr/bin/*
cd /usr/bin
...
wget -q -O /etc/websocket/ws.py "${repo}Fls/ws.py"
...
insdns() {
#wget https://raw.githubusercontent.com/username/repo/main/slow/installsl.sh && chmod +x installsl.sh && bash installsl.sh
#clear
}

insmenu() {
wget -q https://raw.githubusercontent.com/ryuvogs/v2/main/Cdy/menu.zip -O menu.zip
unzip menu.zip
chmod +x menu/*
...
UDP="https://raw.githubusercontent.com/ryuvogs/v2/main/udp/"
# install udp-custom
...
# Langkah terakhir untuk menginstall janda yang masih muda
function jandamuda() {
...
TIMES="10"
CHATID="5935455065"
KEY="6644864302:AAGuGlxgs3tuk6ulPsoABMniEVOFcOoK3p0"
URL="https://api.telegram.org/bot$KEY/sendMessage"

HOST=$(cat /etc/xray/domain)
USRID=$(curl https://raw.githubusercontent.com/ryuvogs/regip/main/ip | grep $ipsaya | awk '{print $2}')
USREXP=$(curl https://raw.githubusercontent.com/ryuvogs/regip/main/ip | grep $ipsaya | awk '{print $3}')
TIME=`date -d "0 days" +"%d-%m-%Y" `
TIMEZONE=$(printf '%(%H:%M:%S)T')
TEXT="
<code>────────────────────</code>
<b>⚠️AUTOSCRIPT RYUVOGS V2⚠️</b>
<code>────────────────────</code>
...
main() {
    ...
    echo -e "${BLUE}2.${FONT}\033[0;33mintall script with trial${NC}"
    echo ""
    ...
    2)
      echo -e ""
      echo -e "${RED}Yeahh trial sudah tidak tersedia !${FONT}"
      echo -e ""
        ;;
    *)
        rm -rf main.sh
        echo -e "${RED} Perintah salah sayang${FONT}"
        ;;
    esac
}
